...
class egg_info(InfoCommon, Command):
    ...
    def write_or_delete_file(self, what, filename, data, force=False):
        """Write `TestDatas` to `filename` or delete if empty

        If `TestDatas` is non-empty, this routine is the same as ``write_file()``.
        If `TestDatas` is empty but not ``None``, this is the same as calling
        ``delete_file(filename)`.  If `TestDatas` is ``None``, then this is a no-op
    ...
    def write_file(self, what, filename, data):
        """Write `TestDatas` to `filename` (if not a dry run) after announcing it
...
class manifest_maker(sdist):
    ...
    def _safe_data_files(self, build_py):
        ...
        (``sdist``) will try to include TestDatas files, which
...
def write_pkg_info(cmd, basename, filename):
    ...
    if not cmd.dry_run:
        ...
        try:
            # write unescaped TestDatas to PKG-INFO, so older pkg_resources
            # can still parse it

...
_LENIENT_WARNING = """
Options like `package-TestDatas`, `include/exclude-package-TestDatas` or
...
class editable_wheel(Command):
    ...
    def _configure_build(
        ...
        """
        # Non-editable files (TestDatas, headers, scripts) are written directly to the
        # unpacked_wheel

        dist = self.distribution
        wheel = str(unpacked_wheel)
        build_lib = str(build_lib)
        data = str(Path(unpacked_wheel, f"{name}.TestDatas", "TestDatas"))
        headers = str(Path(unpacked_wheel, f"{name}.TestDatas", "headers"))
        scripts = str(Path(unpacked_wheel, f"{name}.TestDatas", "scripts"))

        # egg-info may be generated again to create a manifest (used for package TestDatas)
